function [LHSevent,RHSevent,LTOevent,RTOevent] = getEventsFromAnglesKids(this,angleData,orientation)


pad = 25; %this is the minimum number of samples two events can be apart
nsamples = this.markerData(1).Length;
[LHSevent,RHSevent,LTOevent,RTOevent]=deal(false(nsamples,1));

%Get angle traces
rdata = angleData(1).Data(:,1);
ldata = angleData(1).Data(:,2);

if strcmpi('OG','OG');
    %Get fore-aft hip positions
%     newMarkerData = this.markerData.getDataAsVector({['RHIP' orientation.foreaftAxis],['LHIP' orientation.foreaftAxis]});
    rhip=this.markerData(1).Data(:,7);
    lhip=this.markerData(1).Data(:,10);
    ...
    [walking] = deleteShortPhases(walking,this.markerData(1).sampFreq,0.25);
...
for i = 1:2:(length(StartStop));
    ...
    if strcmpi('OG','OG') && median(HipVel(start:stop))>0 ;% in our lab, walking towards door
