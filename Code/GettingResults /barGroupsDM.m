function barGroupsDM(Study,results,groups,params,epochs,indivFlag,colorOrder,mode)
...
% ah=optimizedSubPlot(numPlots,numE,length(params),'lr',12,10,12);
i=1;
...
%        axes(ah(a(i)))
       ...
       hold on
       for b=1:ngroups
           nSubs=length(Study.(groups{b}).IDs);
...
%        set(gca,'Xtick',1:ngroups,'XTickLabel',groups,'fontSize',10)
        set(gca,'Xtick',1:ngroups,'XTickLabel',groupslabels,'fontSize',10)
       xtickangle(45)
       axis tight
...
%    set(ah(a((p*numE-(numE-1):p*numE))),'Ylim',[min(limy) max(limy)])
