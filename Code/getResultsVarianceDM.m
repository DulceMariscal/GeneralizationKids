function results = getResultsVarianceDM(SMatrix,params,groups,maxPerturb,plotFlag,indivFlag,removeBias,cond,maxParam,outlier,mergeG)
...
%                 if length(OGbaseData)<10
%                     OGbase(s,:)=smoothedMax(OGbaseData(1:end,:),transientNumPts,MaxData(1:end));
%                 else
%                     OGbase(s,:)=smoothedMax(OGbaseData(1:10,:),transientNumPts,MaxData(1:10));
%                 end
                 if strcmp(subjectsID{s},'LN0319')
                    OGbase(s,:)= nanvar(OGbaseData(1:end,:));
                 elseif strcmp(subjectsID{s},'LN0341')
                    OGbase(s,:)= nanvar(OGbaseData(end-4:end,:)); 
                 else
                 OGbase(s,:)= nanvar(OGbaseData(1:end,:));
                 end 
            end
            ...
            % compute OG after as var values during strides which cause a
...
%                 OGafter(s,:)= smoothedMax(ogafterData(1:10,:),transientNumPts,MaxData(1:10));
                OGafterLate20(s,:)=nanvar(ogafterData((end-5)-20+1:(end-5),:));
                OGafterLate10(s,:)=nanvar(ogafterData((end-5)-10+1:(end-5),:));
            end
            ...
            TMbase(s,:)=nanvar(TMbaseData(1:end,:)); 
%             TMbase(s,:)=smoothedMax(TMbaseData,transientNumPts,MaxData);
            
            % compute catch as var value during strides which caused a
            ...
            TMafterEarly(s,:)=nanvar(tmafterData(transientNumPts+1:transientNumPts+20,:));
           TMafterLate(s,:)=nanvar(tmafterData((end-5)-steadyNumPts+1:(end-5),:));
            ...
            if nansum(cellfun(@(x) strcmp(x, 'OGBase'), cond))==1
                OGbaseData=adaptData(adaptData(:,2)==OGBaseidx,paramsIdx);
                OGbase(s,:)= nanvar(OGbaseData(1:end,:)); %Average all strides 
                
%                 OGbase(s,:)= nanvar(OGbaseData(1:transientNumPts,:));
%                 if length(OGbaseData)>40
%                 OGbase(s,:)= nanvar(OGbaseData((end-5)-steadyNumPts+1:(end-5),:));
%                 elseif length(OGbaseData)>5
%                  OGbase(s,:)= nanvar(OGbaseData((end-5),:)); 
%                 elseif length(OGbaseData)<5
%                  OGbase(s,:)= nanvar(OGbaseData((end-4),:)); 
%                 end
            end
            
            if nansum(cellfun(@(x) strcmp(x, 'TMBase'), cond))==1
                TMbaseData=adaptData(adaptData(:,2)==TMBaseidx,paramsIdx);
                TMbase(s,:)=nanvar(TMbaseData(1:end,:));  %Average all strides 
%                 TMbase(s,:)=nanvar(TMbaseData(1:transientNumPts,:));
%                 TMbase(s,:)=nanvar(TMbaseData((end-5)-steadyNumPts+1:(end-5),:));
            end
            ...
            if nansum(cellfun(@(x) strcmp(x, 'catch'), lower(cond)))==1
                ...
                elseif size(tmcatchData,1)<3
                    newtmcatchData=nanvar(tmcatchData);
                else
                    if length(tmcatchData)<5
                    newtmcatchData=nanvar(tmcatchData(1:end,:));
                    else
                        newtmcatchData=nanvar(tmcatchData(1:catchNumPts,:));
                    end
                    %newtmcatchData=nanvar(tmcatchData);
                end
            ...
            if nansum(cellfun(@(x) strcmp(x, 'OGpost'), cond))==1
                ogafterData=adaptData(adaptData(:,2)==OGPostidx,paramsIdx);
                OGafter(s,:)=nanvar(ogafterData(1:transientNumPts,:));
                OGafterEarly(s,:)=nanvar(ogafterData(transientNumPts+1:transientNumPts+20,:));
%                 OGafterLate40(s,:)=nanvar(ogafterData((end-5)-40+1:(end-5),:));
                OGafterLate20(s,:)=nanvar(ogafterData((end-5)-20+1:(end-5),:));
                OGafterLate10(s,:)=nanvar(ogafterData((end-5)-10+1:(end-5),:));
                
%                 if length(ogafterData)<50
%                 OGafterLate(s,:)=nanvar(ogafterData((end-5)-20+1:(end-5),:)); %Last strides
%                 else
%                 OGafterLate(s,:)=nanvar(ogafterData((end-5)-steadyNumPts+1:(end-5),:));
%                 end
                
                %Sum of OG after-effects
                AvgOGafter(s,:)=var(ogafterData(1:min([end 50])));
            end
            
            % compute TM post
            if nansum(cellfun(@(x) strcmp(x, 'TMpost'), cond))==1
                tmafterData=adaptData(adaptData(:,2)==TMpostidx, paramsIdx);
                TMafter(s,:)=nanvar(tmafterData(1:transientNumPts,:));
                TMafterEarly(s,:)=nanvar(tmafterData(transientNumPts+1:transientNumPts+20,:));
                TMafterLate(s,:)=nanvar(tmafterData((end-5)-steadyNumPts+1:(end-5),:));
            end
        end
        ...
        if nansum(cellfun(@(x) strcmp(x, 'catch'), lower(cond)))==1
            % compute TM steady state before catch (var of first transinetNumPts of last transinetNumPts+5 strides)
            ...
            Pert(s,:)=nanvar(adapt1Data(1:transientNumPts,:));
            if length(adapt2Data)>20
            LateAdaptBeforeCatch(s,:)=nanvar(adapt2Data((end-5)-steadyNumPts+1:(end-5),:)); %20 strides before catch execpt the very last 5
            else
            LateAdaptBeforeCatch(s,:)=nanvar(adapt2Data(1:end)); %20 strides before catch execpt the very last 5    
            end
...
%             end
%             if ~isempty(idx)
                AdaptIndexBeforeCatch(s,:)=nanvar(adapt2Data((end-5)-steadyNumPts+1:(end-5),:))-Pert(s,:);
%             end
            
            % compute average adaptation value before the catch
            AvgAdaptBeforeCatch(s,:)= nanvar(adapt1Data);
        end
        
        %
        
        % compute TM steady state before OG walking (var of first steadyNumPts of last steadyNumPts+5 strides)
        adapt2Data=[];
...
%         LateAdaptReAdapt(s,:)= nanvar(adapt2Data((end-5)-steadyNumPts+1:(end-5),:)); %last 20 straides of adaptation
          LateAdaptReAdapt(s,:)= nanvar(adapt2Data(end-steadyNumPts+1:end,:));
%         idx = find(strcmpi(params, 'stepLengthAsym'));
%         if isempty(idx)
%             idx = find(strcmpi(params, 'netContributionNorm'));
%         end
%         if ~isempty(idx)
%             AdaptIndex(s,idx)=nanvar(adapt2Sasym((end-5)-steadyNumPts+1:(end-5),:)-adapt2Velocity((end-5)-steadyNumPts+1:(end-5),:));
           AdaptIndex(s,:)=nanvar(adapt2Data((end-5)-steadyNumPts+1:(end-5),:))-Pert(s,:);
%         end
        
        AdaptExtent(s,:)=nanvar(adapt2Sasym((end-5)-steadyNumPts+1:(end-5),:)-adapt2Velocity((end-5)-steadyNumPts+1:(end-5),:));
        ...
        AvgAdaptAll(s,:)= nanvar(adaptAllData);
