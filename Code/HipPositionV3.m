...
function [stepLengthAsym2,stepLengthDiffr,stepLengthSlowr,stepLengthFastr]=HipPositionV3(OverGroundFlag,subjects)
...
%                     LANKX0=
                    ...
                    LANKX0=LHS2x-LHS1x;
                    LANKZ0=LHS2z-LHS1z;
                    LANKVector=[LANKZ0 LANKX0];
%                     reference=[0 HipX0];
                    reference2=[0 LANKX0];
%                     Theta=acos((dot(reference',HipVector'))/(norm(HipVector')*norm(reference')))
                    Theta=acos((dot(reference2,LANKVector))/(norm(LANKVector)*norm(reference2)));
                    
                    
%                     deg2=Theta*(180/pi)
                
                     if (reference2(1,1)>LANKVector(1,1))&&(LANKVector(1,2)<0) || (reference2(1,1)<LANKVector(1,1))&&(LANKVector(1,2)>0) 
                         Theta=Theta;
                     else
                         Theta=-Theta;
                     end
                    ...
                    LHS1vec=[0 LHS1z-LHS1z;0 LHS1x-LHS1x];
                    RHS1vec=[0 RHS1z-LHS1z;0 RHS1x-LHS1x];
                    LHS2vec=[0 LHS2z-LHS1z;0 LHS2x-LHS1x];
                    
                    LAnkleTraZ=LAnklePositionZ'-LHS1z;
                    LAnkleTraX=LAnklePositionX'-LHS1x; 
                    LAnkleTra=[LAnkleTraZ;LAnkleTraX];
                    
                    RAnkleTraZ=RAnklePositionZ'-LHS1z;
                    RAnkleTraX=RAnklePositionX'-LHS1x; 
                    RAnkleTra=[RAnkleTraZ;RAnkleTraX];
                    
%                     LeadX=LAnklePositionX'-LHS1x;
%                     LeadZ=LAnklePositionZ'-LHS1z; 
%                     Lead0=[LeadZ;LeadX];
                    
                    HipTra=[HipZ'-LHS1z;HipX'-LHS1x];
                    %
                    %                    rotHipvect=R*HipVector;
                    rotHipvect=R*[HipLHS1z-LHS1z HipLHS2z-LHS1z;HipLHS1x-LHS1x HipLHS2x-LHS1x];
                    rotLHS1=R*LHS1vec;
                    rotRHS1=R*RHS1vec;
                    rotLHS2=R*LHS2vec;
...
%                     rotLead=R*Lead0;
                    ...
                    figure
                    hold on
%                     plot([0 LANKVector(1)],[0 LANKVector(2)],'--','Displayname','LANKVector')
%                     plot([0 HipVector(1)],[0 HipVector(2)],'--','Displayname','HipVector') %HipVector
%                     plot([0 0],[0 HipX0],'m','Displayname','reference') %reference
                    plot([0 0],[0 LANKX0],'m','Displayname','reference')   
%                     plot(HipVector2(:,1),HipVector2(:,2),'g','Displayname','HipVectorNormal') %HipVector no refrence substracted
                    plot(LHS1z-LHS1z,LHS1x-LHS1x,'<k','Displayname','LHS1')
                    plot(RHS1z-LHS1z,RHS1x-LHS1x,'<r','Displayname','RHS1')
                    plot(LHS2z-LHS1z,LHS2x-LHS1x,'<b','Displayname','LHS2')
                    plot(LAnklePositionZ-LHS1z', LAnklePositionX'-LHS1x,'--b','Displayname','Left angle trayectory')
                    plot(RAnklePositionZ'-LHS1z, RAnklePositionX'-LHS1x,'--b','Displayname','Right angle trayectory')
...
%                       rotLAnkle=[rotLAnkle(2,:)]';
%                       rotRAnkle=[rotRAnkle(2,:)]';
...
%                     plot(rotHipTra)
                    ...
                    legend('Slow leg','Fast leg','SHS','FHS')
